...
class TestReservationDashboard(ReservationCase):
    ...
    def test_customer_dashboard_groups_on_stored_move_partner(self):
        move = self.make_move(10)
        move.partner_id = self.env["res.partner"].create({"name": "Reservation Customer"})
        move._action_assign()

        rows = self.env["stock.reservation.control"].dashboard({}, "customer", 0, 80)["rows"]

        row = next(row for row in rows if row["id"] == move.partner_id.id)
