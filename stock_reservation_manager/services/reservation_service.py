...
class ReservationService(models.AbstractModel):
    ...
    def get_dashboard(self, filters, tab, offset, limit):
        ...
        if tab == "conflict":
            ...
            rows = self._customer_rows(line_domain, offset, limit)
        elif tab == "warehouse":
    ...
    def _customer_rows(self, domain, offset, limit):
        groups = self.env["stock.move.line"]._read_group(
            domain + [("picking_partner_id", "!=", False)], ["picking_partner_id"],
            ["quantity_product_uom:sum", "product_id:count_distinct", "picking_id:count_distinct"],
            offset=offset, limit=limit, order="quantity_product_uom:sum desc")
